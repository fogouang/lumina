  ...
  <div class="min-h-screen bg-(--bg-ground)">
    ...
    <div class="relative bg-linear-to-br from-primary-700 to-primary-900 py-14 text-center overflow-hidden">
      ...
      <div class="relative z-10 container mx-auto px-4">
        ...
        <div v-if="auth.isAuthenticated" class="inline-flex items-center gap-2 mt-5 bg-white/20 rounded-xl px-4 py-2">
          ...
          <span class="text-white font-semibold text-sm">{{ sub.aiCreditsRemaining }} crédit{{ sub.aiCreditsRemaining > 1 ? 's' : '' }} IA disponible{{ sub.aiCreditsRemaining > 1 ? 's' : '' }}</span>
        </div>
      </div>
    </div>
    ...
    <div class="container mx-auto px-4 py-10 max-w-5xl">
      ...
      <template v-else>
        ...
        <!-- Crédits faibles -->
        <div v-if="sub.aiCreditsRemaining === 0" class="flex items-start gap-4 bg-amber-50 border border-amber-200 rounded-xl p-5 mb-8">
          ...
          <div class="flex-1">
            ...
            <p class="text-sm text-amber-700 leading-relaxed">Vous pouvez lire les sujets mais pas lancer le simulateur avec correction. Achetez des crédits pour obtenir une correction en temps réel.</p>
          </div>
            ...
            <div class="px-5 py-4 flex-1 flex flex-col gap-4">
              ...
              <div class="mt-auto pt-2 flex flex-col gap-2">
                <NuxtLink :to="`/simulateur/expression-ecrite/${session.id}`">
                  <Button
                    :label="sub.aiCreditsRemaining > 0 ? 'Simuler' : 'Voir les sujets'"
        ...
        <!-- Info crédits -->
        <div class="mt-10 bg-(--bg-card) border border-(--border-color) rounded-2xl p-6 flex flex-col md:flex-row items-start md:items-center gap-4">
          <div class="flex items-center gap-3 flex-1">
            <div class="w-12 h-12 rounded-xl bg-purple-50 flex items-center justify-center shrink-0">
              <i class="pi pi-sparkles text-purple-500 text-xl" />
            </div>
            <div>
              <p class="font-bold text-(--text-primary) mb-0.5">Comment fonctionnent les crédits IA ?</p>
              <p class="text-sm text-(--text-secondary) leading-relaxed">
                1 crédit = 1 correction IA pour une tâche. Chaque simulation des 3 tâches consomme 3 crédits.
              </p>
            </div>
          </div>
