  ...
  <div class="dashboard">
    ...
    <div class="account-section">
      <div class="dashboard__section-header">
        <h2 class="account-section__title" style="margin:0;border:none;padding:0;">
          Tentatives récentes
        </h2>
      ...
      <div v-else-if="!recentAttempts.length" class="dashboard__empty">
        <i class="pi pi-inbox" />
        <p>Aucune tentative. <NuxtLink to="/epreuve/comprehension-ecrite/series">Commencer un test</NuxtLink></p>
      </div>
      ...
      <div v-else class="dashboard__attempts">
        ...
        >
          <div class="dashboard__attempt-icon" :class="`dashboard__attempt-icon--${attempt.status}`">
            <i :class="statusIcon(attempt.status)" />
          </div>
          <div class="dashboard__attempt-body">
            <span class="dashboard__attempt-serie">Série {{ attempt.series_number ?? '—' }}</span>
            <div class="dashboard__attempt-meta">
              <Tag :value="statusLabel(attempt.status)" :severity="statusSeverity(attempt.status)" />
              <span v-if="attempt.oral_score || attempt.written_score" class="dashboard__attempt-score">
                {{ attempt.oral_score ?? attempt.written_score }}/699
              </span>
              <span class="dashboard__attempt-date">{{ formatDate(attempt.started_at) }}</span>
            </div>
          </div>
          <NuxtLink
    ...
    <div class="account-section">
      ...
      <div class="dashboard__quick">
        ...
        >
          ...
          <span>{{ ep.title }}</span>
          <i class="pi pi-angle-right" style="margin-left:auto;color:var(--text-tertiary)" />
        </NuxtLink>
      </div>
    </div>
...
const completedCount  = computed(() => allAttempts.value.filter(a => a.status === 'completed').length)
const inProgressCount = computed(() => allAttempts.value.filter(a => a.status === 'in_progress').length)
...
const avgScore = computed(() => {
  ...
  const total = completed.reduce((sum, a) => sum + (a.oral_score ?? a.written_score ?? 0), 0)
  return Math.round(total / completed.length)
})

function statusLabel(status: string) {
  return { in_progress: 'En cours', completed: 'Terminé', abandoned: 'Abandonné' }[status] ?? status
}
function statusSeverity(status: string) {
  return { in_progress: 'warning', completed: 'success', abandoned: 'danger' }[status] ?? 'secondary'
}
function statusIcon(status: string) {
  return { in_progress: 'pi pi-clock', completed: 'pi pi-check', abandoned: 'pi pi-times' }[status] ?? 'pi pi-circle'
}
...
const epreuves = [
  { slug: 'comprehension-ecrite', title: 'Compréhension Écrite', icon: 'pi pi-book',          iconColor: '#1d4ed8', iconBg: '#eff6ff' },
  { slug: 'comprehension-orale',  title: 'Compréhension Orale',  icon: 'pi pi-headphones',    iconColor: '#0d9488', iconBg: '#f0fdfa' },
  { slug: 'expression-ecrite',    title: 'Expression Écrite',    icon: 'pi pi-pen-to-square', iconColor: '#7c3aed', iconBg: '#f5f3ff' },
  { slug: 'expression-orale',     title: 'Expression Orale',     icon: 'pi pi-microphone',    iconColor: '#d97706', iconBg: '#fffbeb' },
]
...
.stat-card__icon i { font-size: 1.25rem; }

.stat-card__icon--primary { background: var(--color-primary-50); }
.stat-card__icon--primary i { color: var(--color-primary-600); }

.stat-card__icon--warning { background: #fffbeb; }
.stat-card__icon--warning i { color: #d97706; }

.stat-card__icon--success { background: #f0fdf4; }
.stat-card__icon--success i { color: #16a34a; }

.stat-card__icon--purple { background: #f5f3ff; }
.stat-card__icon--purple i { color: #7c3aed; }
...
.dashboard__see-all i { font-size: 0.75rem; }
...
.dashboard__empty i { font-size: 1.25rem; }
...
.dashboard__attempt:last-child { border-bottom: none; }
...
.dashboard__attempt-icon--completed   { background: #f0fdf4; }
.dashboard__attempt-icon--completed i { color: #16a34a; }
.dashboard__attempt-icon--in_progress   { background: #fffbeb; }
.dashboard__attempt-icon--in_progress i { color: #d97706; }
.dashboard__attempt-icon--abandoned   { background: var(--color-danger-50); }
.dashboard__attempt-icon--abandoned i { color: var(--color-danger-600); }

.dashboard__attempt-body { flex: 1; min-width: 0; }
...
.dashboard__quick-item:last-child { border-bottom: none; }

.dashboard__quick-item:hover { color: var(--color-primary-700); }
...
.dashboard__quick-icon i { font-size: 1rem; }
...
@media (max-width: 640px) {
  ...
  .dashboard__subscription { flex-direction: column; align-items: flex-start; }
}
</style>