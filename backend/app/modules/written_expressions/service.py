...
class WrittenExpressionService:
    ...
    #     # Vérifier qu'il n'y a pas déjà de correction IA
    #     existing = await self.ai_correction_repo.get_by_expression(expression_id)
    #     if existing:
    #         raise BadRequestException(detail="Cette expression a déjà une correction IA")
        
    #     # Vérifier et consommer crédits IA
    #     from app.shared.utils.ai_credits import AICreditManager
        
    #     credit_manager = AICreditManager(self.db)
    #     credit_result = await credit_manager.check_and_consume_credit(
    #         user_id=current_user.id,
    #         cost=1  # 1 crédit par correction
    #     )
    
    #     # Récupérer la tâche pour les contraintes
    #     task = await self.task_repo.get_by_id_or_404(expression.task_id)
        
    #     # Appeler l'IA
    #     ai_corrector = AICorrector()
        
    #     result = await ai_corrector.correct_written_expression(
    #         text=expression.text_content,
    #         task_instruction=task.instruction_text,
    #         word_count_min=task.word_count_min or 50,
    #         word_count_max=task.word_count_max or 150
    #     )
        
    #     # Créer la correction
    #     ai_correction = await self.ai_correction_repo.create(
    #         expression_id=expression_id,
    #         corrected_text=result.get("corrected_text", ""),
    #         structure_score=result.get("structure_score"),
    #         structure_feedback=result.get("structure_feedback"),
    #         cohesion_score=result.get("cohesion_score"),
    #         cohesion_feedback=result.get("cohesion_feedback"),
    ...
    #         cecrl_level=result.get("cecrl_level"),
    #         appreciation=result.get("appreciation"),
    #         corrections_json={"corrections": result.get("corrections", [])},
    #         suggestions_json={"suggestions": result.get("suggestions", [])}
    #     )
        
    #     # Mettre à jour le statut de l'expression
    #     await self.repo.update(
    #         expression_id,
    #         correction_status=CorrectionStatus.CORRECTED_AI
    #     )
    ...
    #     # Notifier si crédits faibles
    #     if credit_result["notification_sent"]:
    #         await notif_service.notify_credits_low(
    #             user_id=current_user.id,
    #             credits_remaining=credit_result["credits_remaining"]
    #         )
